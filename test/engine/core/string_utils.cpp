...
TEST_CASE("split string")
{
    ...
    SUBCASE("default params") {
        std::string e = "tok1,tok2,tok3,tok4";
        std::vector<std::string_view> v = split(e);
        REQUIRE(v.size() == 4);
        REQUIRE((v[0] == "tok1"));
        REQUIRE((v[1] == "tok2"));
    ...
    SUBCASE("custom multiple params") {
        std::string e = "tok1 tok2\ntok3\ttok4.tok5";

        constexpr std::string_view delims[] = {" ", "\n", "\t", "."};
        std::vector<std::string_view> v = split(e, delims);
        REQUIRE(v.size() == 5);
        REQUIRE((v[0] == "tok1"));
        REQUIRE((v[1] == "tok2"));
        REQUIRE((v[2] == "tok3"));
    ...
    SUBCASE("custom multiple params out of order") {
        std::string e = "tok1 tok2\ntok3\ttok4.tok5";
...
TEST_CASE("join string")
{
    ...
    SUBCASE("join with begin end") {
        std::array strs{"a", "b", "c", "d"};
        std::string joined = join(strs, /*delim=*/"|", /*begin=*/"{", /*end=*/"}");
        REQUIRE_EQ(joined, "{a|b|c|d}");
    }
}
